...
class ViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row < voivodships.count {
            let cell = tableView.dequeueReusableCell(withIdentifier: "cellVoivodship", for: indexPath) as! VoivodshipTableViewCell
            let voivodshipName = voivodships[indexPath.row] // <-- Fix this

            cell.labelVoivodship.text = voivodshipName

            let selectors = getSelectors(for: voivodshipName)
            cell.labelSelectors.text = arrayToString(strings: selectors)

            if let firstSelector = selectors.first {
                cell.voivodshipImg.image = UIImage(named: firstSelector)
            } else {
                cell.voivodshipImg.image = nil
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "segueSearchResults":
            if let destination = segue.destination as? DetailsViewController {
                ...
                destination.selectedVoivodship = selectedVoivodship
                destination.selectedDistrict = foundDistrict
            }
        default:
            return
        }
